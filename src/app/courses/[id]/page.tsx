        ...
        <div className="flex flex-col gap-4">
          {course.lessons.map((lesson, index) => {
            ...
            <Link 
              ...
              className="group flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6 p-4 sm:p-5 bg-white dark:bg-slate-900 rounded-2xl shadow-sm hover:shadow-md hover:shadow-indigo-500/5 transition-all border border-slate-200 dark:border-slate-800 hover:border-indigo-300 dark:hover:border-indigo-500/50 relative overflow-hidden"
            >
              ...
              <div className="flex items-center gap-4 sm:gap-6 flex-grow">
                {/* Number indicator */}
                <div className="flex-shrink-0 w-12 h-12 flex items-center justify-center bg-slate-50 dark:bg-slate-950 text-slate-400 dark:text-slate-500 font-black text-lg rounded-full group-hover:bg-indigo-50 dark:group-hover:bg-indigo-900/40 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                  {index + 1}
                </div>
                
                <div className="flex-grow">
                  <h3 className="text-base sm:text-lg font-bold text-slate-900 dark:text-slate-100 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors mb-1">
                    {lesson.title}
                  </h3>
              ...
              <div className="flex-shrink-0 pl-16 sm:pl-0">
                {isCompleted ? (
                  ...
                  <span className="px-3.5 py-1.5 bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400 text-xs font-bold uppercase tracking-wider rounded-full flex justify-center w-fit">
                    Chưa học
                  </span>
                )}
              </div>
